...
<div class="w-full flex flex-col gap-6 animate-fade-in pb-12 p-8 max-w-6xl mx-auto">
  ...
  <div class="glass p-6 rounded-2xl border border-border/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
      <h1 class="text-xl md:text-2xl font-extrabold text-verdigris-400">Direktori Pelaku Sejarah</h1>
  ...
  <div class="glass p-4 rounded-xl border border-border/10 flex flex-col md:flex-row gap-4 items-center justify-between">
    <div class="relative w-full md:w-80">
      <input
        ...
        class="w-full bg-iron-950/60 border border-border/15 rounded-xl px-4 py-2.5 text-xs text-text-primary placeholder-text-muted outline-none focus:border-verdigris-500/40 transition-colors"
      />
    </div>
    <div class="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
      ...
      <div class="flex items-center bg-iron-950/60 border border-border/10 rounded-lg p-1">
        <button 
          class="p-1.5 rounded-md text-xs transition-colors {viewMode === 'grid' ? 'bg-verdigris-500/20 text-verdigris-400' : 'text-text-muted hover:text-text-primary'}"
        ...
        <button 
          class="p-1.5 rounded-md text-xs transition-colors {viewMode === 'list' ? 'bg-verdigris-500/20 text-verdigris-400' : 'text-text-muted hover:text-text-primary'}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each filteredActors as actor}
        <a 
          ...
          class="glass p-5 rounded-2xl border border-border/10 hover:border-verdigris-500/30 hover:shadow-[0_0_20px_rgba(16,185,129,0.05)] transition-all flex flex-col gap-4 group"
        >
          <div class="flex justify-between items-start">
            <div class="w-12 h-12 rounded-xl bg-verdigris-500/10 border border-verdigris-500/20 flex items-center justify-center text-xl text-verdigris-400 group-hover:scale-105 transition-transform">
              👤
            </div>
          ...
          <div>
            <h3 class="text-sm font-bold text-text-primary group-hover:text-verdigris-400 transition-colors">{actor.name}</h3>
          ...
          <div class="flex flex-wrap gap-1 mt-auto">
            {#each (actor.roles || []).slice(0, 2) as role}
              <span class="px-2 py-0.5 rounded bg-iron-950/60 border border-border/10 text-verdigris-400 text-[9px] font-mono">{role}</span>
            {/each}
            {#each (actor.works || []).slice(0, 1) as work}
              <span class="px-2 py-0.5 rounded bg-iron-950/60 border border-border/10 text-gold-400 text-[9px] italic">📖 {work}</span>
            {/each}
          </div>
        </a>
      {/each}
  ...
  {:else}
    <div class="flex flex-col gap-3">
      {#each filteredActors as actor}
        <a 
          href="/actor/{actor.uuid}"
          class="glass p-4 rounded-xl border border-border/10 hover:border-verdigris-500/30 hover:shadow-[0_0_20px_rgba(16,185,129,0.05)] transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 group"
        >
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-verdigris-500/10 border border-verdigris-500/20 flex items-center justify-center text-lg text-verdigris-400 group-hover:scale-105 transition-transform flex-shrink-0">
              👤
            </div>
            <div>
          ...
          <div class="flex items-center gap-4 md:gap-6 flex-wrap text-[10px] justify-between md:justify-end">
            ...
            <div class="flex flex-wrap gap-1.5 max-w-md">
              {#each (actor.roles || []).slice(0, 3) as role}
                <span class="px-2 py-0.5 rounded bg-iron-950/60 border border-border/10 text-text-muted text-[9px]">{role}</span>
              {/each}
              {#if (actor.roles || []).length > 3}
                <span class="px-2 py-0.5 rounded bg-verdigris-500/10 border border-verdigris-500/20 text-verdigris-400 text-[9px]">+{actor.roles.length - 3}</span>
              {/if}
            </div>
          </div>
        </a>
