<script lang="ts">
  ...
  function get3DCoords(lng: number, lat: number, year: number): THREE.Vector3 {
    ...
    const z = -(((lat - MIN_LAT) / (MAX_LAT - MIN_LAT)) * MAP_HEIGHT - MAP_HEIGHT / 2); // Negate for latitude direction
  ...
  const locationCoordsRegistry: Record<string, { lat: number; lng: number }> = {
    ...
    'baitul maqdis': { lat: 31.7780, lng: 35.2354 },
    ...
    'mesopotamia': { lat: 32.5000, lng: 44.0000 },
    'alexandria': { lat: 31.2001, lng: 29.9187 },
    'badr': { lat: 23.7744, lng: 38.7903 },
    'chang\'an': { lat: 34.2670, lng: 108.9500 }
  };
  ...
  function getCoordsForEvent(ev: TimelineEvent): THREE.Vector3 {
    ...
    if (ev.locations && ev.locations.length > 0) {
      ...
      const matchedKey = Object.keys(locationCoordsRegistry).find(k => locName.includes(k) || k.includes(locName));
      if (matchedKey) {
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      if (intersects.length > 0) {
        ...
        if (hoveredObject !== object) {
          ...
          // Highlight color
          (object as any).material.color.set('#34d399'); // Emerald light
          (object as any).material.emissive.set('#047857');
...
<div class="relative w-full h-full flex flex-col min-h-[450px]">
  ...
  <div class="absolute bottom-6 left-6 z-10 glass p-4 rounded-xl border border-border/15 flex flex-col gap-4 pointer-events-none shadow-[0_8px_30px_rgba(0,0,0,0.6)] backdrop-blur-md min-w-[240px]">
    
    <!-- Legend Section -->
    <div class="flex flex-col gap-2.5">
      <span class="text-[9px] font-bold text-text-secondary uppercase tracking-widest block border-b border-border/10 pb-1.5">
        Legend Space-Time Cube
      </span>
      <div class="flex items-center gap-2.5 text-[9px]">
        <span class="w-2.5 h-2.5 bg-gold-500 rounded-sm shadow-[0_0_6px_rgba(212,168,83,0.8)] inline-block"></span>
      ...
      <div class="flex items-center gap-2.5 text-[9px]">
        <span class="w-3.5 h-0.5 border-t border-dashed border-slate-500 inline-block"></span>
        <span class="text-text-primary font-medium">Proyeksi Geografis Bumi</span>
      </div>
    </div>

    <!-- Navigation Section -->
    <div class="flex flex-col gap-2 pt-2 border-t border-border/10">
      <span class="text-[9px] font-bold text-gold-400 uppercase tracking-widest block mb-0.5 flex items-center gap-1.5">
